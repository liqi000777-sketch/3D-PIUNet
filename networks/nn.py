...
def mean_flat_nodemask(tensor, node_mask):
    """
    Take the mean over all non-batch dimensions.
    """
    return (tensor * node_mask).sum(dim=list(range(1, len(tensor.shape)))) / node_mask.sum()
...
def init_weights(net, init_type="xavier", init_gain=0.1, activation="selu"):
